...
def cancel_order_in_inpost(inpost_order_id):
    ...
    try:
        response = requests.delete(url, headers=inpost_headers)
...
def check_and_sync_cancelled_orders():
    ...
    cancelled_orders = get_recent_cancelled_orders(7)
...
def get_recent_cancelled_orders(days_back=7):
    """Get recently cancelled/voided orders from CIN7"""
    start_date = (datetime.now() - timedelta(days=days_back)).strftime("%Y-%m-%d")
    
    # First try to get Voided orders
    voided_orders = []
    try:
        ...
        if data.get("SaleList"):
            for sale in data["SaleList"]:
                if sale.get("Status") == "Voided":
                    voided_orders.append(sale)
    except Exception as e:
        print(f"⚠️  Warning: Could not fetch voided orders: {str(e)}")
    
    # You might also want to check for orders that were deleted or have other indicators
    # This would depend on how exactly orders are "cleared out" in your CIN7 setup
    
    print(f"📥 Found {len(voided_orders)} voided orders in CIN7")
