...
def get_recent_sale_ids(days_back=10, limit=1000, page_param="Page"):
    """Fetch recent SaleIDs from Cin7 Core (DEAR) saleList with pagination.

    Notes:
    - Uses the `From` date filter (YYYY-MM-DD).
    - Paginates using `Page` + `Limit` (default), and stops when an empty page is returned.
    - Filters by `relevant_statuses` to avoid pulling historical/closed sales unnecessarily.
    """
    start_date = (datetime.now() - timedelta(days=days_back)).strftime("%Y-%m-%d")
    ...
    while True:
        # Dear/Cin7 Core saleList paging is commonly Page+Limit. If your tenant uses a different param,
        # you can override via the `page_param` argument.
        url = (
            "https://inventory.dearsystems.com/ExternalApi/v2/saleList"
            f"?From={start_date}&Limit={int(limit)}&{page_param}={page}"
        )
        ...
        try:
            ...
            print(f"❌ Error fetching recent sales (page {page}): {str(e)}")
            break

        sale_list = data.get("SaleList") or []
        if not sale_list:
            if page == 1:
                print("❌ No sales found.")
            break

        # Add filtered SaleIDs
        for sale in sale_list:
            if sale.get("Status") in relevant_statuses and sale.get("SaleID"):
                sale_ids.append(sale["SaleID"])

        # Stop when we receive less than a full page
        if len(sale_list) < int(limit):
            break
...
SALE_IDS = get_recent_sale_ids(LOOKBACK_DAYS)
