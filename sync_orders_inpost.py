...
class Cin7Client:
    ...
    def get_orders(self, start_date: datetime, end_date: datetime, status: str = "NOT FULFILLED") -> List[Dict]:
        """Fetch orders from Cin7 within a date range."""
        ...
        try:
            ...
            data = response.json()
            return data.get("SaleList", [])
        except Exception as e:
            logger.error(f"Failed to fetch orders from Cin7: {str(e)}")
            return []
...
class OrderProcessor:
    ...
    def process_new_orders(self):
        ...
        orders = self.cin7.get_orders(start_date, end_date)
    ...
    def build_inpost_courier_payload(self, order_data: Dict, shipping: Dict, country_code: str) -> Dict:
        ...
        payload = {
            ...
            "orderDate": order_data.get("SaleOrderDate", datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
            "carrier": carrier,
...
def main():
    ...
    initial_end = datetime.utcnow()
    initial_start = initial_end - timedelta(hours=Config.INITIAL_LOOKBACK_HOURS)
    
    initial_orders = processor.cin7.get_orders(initial_start, initial_end)
    ...
    # Check for any pending tracking and fulfillment updates
